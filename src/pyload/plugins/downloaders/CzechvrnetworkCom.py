...
class CzechvrnetworkCom(SimpleDownloader):
    __name__ = "CzechvrnetworkCom"
    __type__ = "downloader"
    __version__ = "0.1"
    __status__ = "testing"

    __pattern__ = r"https?://(?:www\.)?czechvrnetwork\.com/(members/)?detail-(?P<ID>\d+)"
    __config__ = [
    ...
    __description__ = """CzechVR Network downloader plugin"""
    __license__ = "GPLv3"
    ...
    def setup(self):
        super(CzechvrnetworkCom, self).setup()
    ...
    def handle_premium(self, pyfile):
        scene_id = self.info['pattern']['ID']

        response_ = self.load(
            f"https://beta.czechvrnetwork.com/heresphere/videoID{scene_id}",
            post=True,
            cookies=False,
            decode=False
        )
